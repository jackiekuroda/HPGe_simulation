...
DetectorConstruction::DetectorConstruction(G4double outerDeadLayerThick)
    : G4VUserDetectorConstruction(),
      physiWorld(nullptr),
      stepLimit(nullptr),
      fCheckOverlaps(true),
      fouterDeadLayerThick(outerDeadLayerThick) //to be used for easy dead layer testing
{
    DefineMaterials();
...
G4double DetectorConstruction::GetouterDeadLayerThick() const
{
   return fouterDeadLayerThick;
}
...
void   DetectorConstruction::ConstructHPGeDetector(G4LogicalVolume* motherLogicalVolume)
{
	...
	G4double outerDeadLayerThick = fouterDeadLayerThick;
